...
def getData():
    ...
    passedNamazes = {}
    closest_prayer = namaz_times['closest_prayer']['namazName']
    if closest_prayer == 'Fajr':
        passedNamazes = {'Fajr': False, 'Dhuhr': False, 'Asr': False, 'Maghrib': False, 'Isha\'a': False}
    elif closest_prayer == 'Dhuhr':
        passedNamazes = {'Fajr': True, 'Dhuhr': False, 'Asr': False, 'Maghrib': False, 'Isha\'a': False}
    elif closest_prayer == 'Asr':
        passedNamazes = {'Fajr': True, 'Dhuhr': True, 'Asr': False, 'Maghrib': False, 'Isha\'a': False}
    elif closest_prayer == 'Maghrib':
        passedNamazes = {'Fajr': True, 'Dhuhr': True, 'Asr': True, 'Maghrib': False, 'Isha\'a': False}
    elif closest_prayer == 'Isha\'a':
        passedNamazes = {'Fajr': True, 'Dhuhr': True, 'Asr': True, 'Maghrib': True, 'Isha\'a': False}
    # create the json file
    with open("./react-app/src/namaz.json", "w") as file:
        json.dump(namaz_times, file, indent=4)

    with open('./react-app/src/passed.json', 'w') as file:
        json.dump(passedNamazes, file, indent=4)
    return namaz_times    
getData()